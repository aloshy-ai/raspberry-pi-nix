...
CMD ["sh", "-c", "nix-shell -p zstd --run 'nix build --option system aarch64-linux --no-update-lock-file \".#nixosConfigurations.rpi-example.config.system.build.sdImage\" && mkdir -p artifacts && chown -R $(id -u):$(id -g) artifacts && cp -f -R -L /nix/store/*nixos-sd-image-* artifacts/ && unzstd -d artifacts/*nixos-sd-image-*-aarch64-linux.img/sd-image/nixos-sd-image-*-aarch64-linux.img.zst -o artifacts/nixos-sd-image-aarch64-linux.img'"]
